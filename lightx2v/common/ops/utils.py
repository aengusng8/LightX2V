...
def get_source_tensor(source_name, weight_dict, lazy_load, lazy_load_file, use_infer_dtype, scale_force_fp32, bias_force_fp32):
    ...
    if lazy_load:
        ...
        # Resolve key: try primary, then .scale as fallback for weight_scale
        key = source_name
        if key not in weight_dict and "weight_scale" in source_name:
            alt_key = source_name.replace(".weight_scale", ".scale")
            if alt_key in weight_dict:
                key = alt_key
        if key not in weight_dict:
            raise KeyError(key)
        tensor = weight_dict[key]
        if use_infer_dtype:
            return tensor.to(GET_DTYPE())
        elif scale_force_fp32 and "weight_scale" in source_name:
...
def create_cuda_buffers(base_attrs, weight_dict, lazy_load, lazy_load_file, use_infer_dtype=None, scale_force_fp32=False, bias_force_fp32=False):
    ...
    for name, attr_name, transpose in base_attrs:
        try:
            ...
            # Missing weight_scale: use default scale 1.0 (checkpoint may use different naming or no separate scale)
            if attr_name == "weight_scale" and "weight" in result:
